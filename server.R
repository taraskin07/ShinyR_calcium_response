...
server <- function(input, output, session) {
  ...
  sheets_in_the_dataTS <-  eventReactive(eventExpr = {
    input$dataTS
  },
  valueExpr = {
  ...
  observeEvent(input$dataTS, {
    ...
    updateSelectInput(
      inputId = 'sheetNum',
      choices = sheets_in_the_dataTS(),
      selected = str_extract(sheets_in_the_dataTS(), '^Numerator$|^340$')
    )
    
    updateSelectInput(
      inputId = 'sheetDen',
      choices = sheets_in_the_dataTS(),
      selected = str_extract(sheets_in_the_dataTS(), '^Denominator$|^380$')
    )
  ...
  observeEvent(input$change_names_button, {
    runjs(
      'document.getElementById("change_names_button").style.backgroundColor = "green";'
    )
  ...
  #  RATIO
  df_ratio_ready <- eventReactive(eventExpr = {
    input$dataTS
    input$disp
    input$correct_time
    input$change_names
  ...
  # NUMERATOR
  df_Num_ready <- eventReactive(eventExpr = {
    input$dataTS
    input$disp
    input$correct_time
    input$change_names
  ...
  # DENOMINATOR
  df_Den_ready <- eventReactive(eventExpr = {
    input$dataTS
    input$disp
    input$correct_time
    input$change_names
  ...
  df_custom_ratio_ready <- eventReactive(eventExpr = {
    input$dataTS
    input$disp
    input$correct_time
    input$change_names
  ...
  output$SaveXlsBox1 <- downloadHandler(
    filename = function() {
      filename(input$dataTS, "ProcessedTable.xlsx")
    },
  ...
  df_Num_basic_stat <- eventReactive(eventExpr = {
    input$basicStat
  },
  valueExpr = {
    req(input$dataTS)
  ...
  df_Den_basic_stat <- eventReactive(eventExpr = {
    input$basicStat
  },
  valueExpr = {
    req(input$dataTS)
  ...
  output$SaveXlsBoxStat <- downloadHandler(
    filename = function() {
      custom_filename(input$dataTS, "BasicStatisticsTable.xlsx")
    },
    content = function(file) {
      wb <- createWorkbook()
      ...
      if (input$cRatio) {
        sheet1 <- addWorksheet(wb, sheetName = "Ratio")
        writeData(wb, sheet1, df_ratio_basic_stat_table())
      }
      
      if (input$cNum) {
        sheet2 <- addWorksheet(wb, sheetName = "Numerator")
        writeData(wb, sheet2, df_Num_basic_stat_table())
      }
      
      if (input$cDen) {
        sheet3 <- addWorksheet(wb, sheetName = "Denominator")
        writeData(wb, sheet3, df_Den_basic_stat_table())
      }
      
      if (input$cNum & input$cDen) {
        sheet4 <- addWorksheet(wb, sheetName = "Custom_ratio")
        writeData(wb, sheet4, df_custom_ratio_basic_stat_table())
      }
  ...
  rmcellValues$colors2000 <-
    randomColor(count = 2000,
                hue = 'random',
                luminosity = 'bright')
  ...
  observeEvent(input$exclude_cell, {
    ...
    if (input$tab == "R") {
      rmcellValues$cList <- unique(c(
        shiny::isolate(rmcellValues$cList),
  ...
  observeEvent(input$exclude_undo, {
    ...
    rmcellValues$cList <-
      rmcellValues$cList[-length(rmcellValues$cList)]
  ...
  observeEvent(input$new_dataframes, {
    ...
    if ((input$correct_time %% 2) == 1) {
      runjs(
        'document.getElementById("new_dataframes").style.backgroundColor = "green";'
      )
    } else {
      runjs(
        'document.getElementById("new_dataframes").style.backgroundColor = "grey";'
      )
    }
  })
  ...
  df_custom_ratio_excluded <-
    ...
    valueExpr = {
      ...
      subset(df_custom_ratio_basic_stat(),
             select = !(
               colnames(df_custom_ratio_basic_stat()) %in% rmcellValues$cList
             ))
  ...
  output$SaveXlsBoxNoBadCells <- downloadHandler(
    filename = function() {
      custom_filename(input$dataTS, "CleanTable.xlsx")
    },
    content = function(file) {
      wb <- createWorkbook()
      ...
      if (input$cRatio) {
        sheet1 <- addWorksheet(wb, sheetName = "Ratio")
        writeData(wb, sheet1, df_ratio_excluded())
      }
      
      if (input$cNum) {
        sheet2 <- addWorksheet(wb, sheetName = "Numerator")
        writeData(wb, sheet2, df_Num_excluded())
      }
      
      if (input$cDen) {
        sheet3 <- addWorksheet(wb, sheetName = "Denominator")
        writeData(wb, sheet3, df_Den_excluded())
      }
      
      if (input$cNum & input$cDen) {
        sheet4 <- addWorksheet(wb, sheetName = "Custom_ratio")
        writeData(wb, sheet4, df_custom_ratio_excluded())
      }
      
      if (!is.null(rmcellValues$cList)) {
        excluded <- data.frame(Excluded_traces = rmcellValues$cList)

      } else {
        excluded <- data.frame(Excluded_traces = character())
      }
      
      sheet5 <- addWorksheet(wb, sheetName = "excluded_cells")
      writeData(wb, sheet5, excluded)
  ...
  observeEvent(input$clean_file, {
    ...
    updateSelectInput(
      inputId = 'sheetClNum',
      choices = sheets_in_a_clean_file(),
      selected = str_extract(sheets_in_a_clean_file(), '^Numerator$|^340$')
    )
    
    updateSelectInput(
      inputId = 'sheetClDen',
      choices = sheets_in_a_clean_file(),
      selected = str_extract(sheets_in_a_clean_file(), '^Denominator$|^380$')
    )
  ...
  # Ratio
  df_ratio_clean <- eventReactive(eventExpr = {
    input$sheetClRatio
    input$clean_file
  },
  valueExpr = {
  ...
  # Num
  df_Num_clean <- eventReactive(eventExpr = {
    input$sheetClNum
  },
  valueExpr = {
    req(input$clean_file)
  ...
  # Den
  df_Den_clean <- eventReactive(eventExpr = {
    input$sheetClDen
  },
  valueExpr = {
    req(input$clean_file)
      ...
      if (input$peaks_amount == 1) {
        ...
        tagList(
          ...
          sliderInput(
            "slider_output2",
            "Range 2:",
            min = min(time_column_name()),
            max = max(time_column_name()),
  ...
  observeEvent(input$plot_all2, {
    output$plot_ratio2 <- renderPlotly({
      ...
      pl <-
        plot_wrapper(
          plot_object,
          input$slider_output1,
          input$slider_output2,
          input$slider_output3,
          input$slider_output4
        )
  ...
  observeEvent(input$plot_single2, {
    output$plot_ratio2 <- renderPlotly({
      ...
      plot_object <-
        display_single_plot(df_ratio_clean(),
                            input$ratioInput2,
                            ready = F,
                            lines = T)
      pl <-
    ...
    output$plotNum2 <- renderPlotly({
      ...
      plot_object <-
        display_single_plot(df_Num_clean(),
                            input$numInput2,
                            ready = F,
                            lines = T)
      pl <-
    ...
    output$plotDen2 <- renderPlotly({
      ...
      plot_object <-
        display_single_plot(df_Den_clean(),
                            input$denInput2,
                            ready = F,
                            lines = T)
      pl <-
  ...
  df_excluded_cells_list <-
    ...
    valueExpr = {
      ...
      if ('excluded_cells' %in% sheet_names) {
        ...
        df <-
          read_excel(input$clean_file$datapath, sheet = 'excluded_cells')
      } else {
  ...
  output$SaveXlsAmpl <- downloadHandler(
    filename = function() {
      filename(input$clean_file, "Amplitudes.xlsx")
    },
    content = function(file) {
      wb <- createWorkbook()
      ...
      if (input$clRatio) {
        sheet1 <- addWorksheet(wb, sheetName = "Ratio")
        writeData(wb, sheet1, df_ratio_amplitude())
        
        sheet2 <- addWorksheet(wb, sheetName = "Ratio_summary")
        writeData(wb, sheet2, df_ratio_summary())
      }
      
      if (input$clNum) {
        sheet3 <- addWorksheet(wb, sheetName = "Numerator")
        writeData(wb, sheet3, df_Num_amplitude())
        
        sheet4 <- addWorksheet(wb, sheetName = "Num_summary")
        writeData(wb, sheet4, df_Num_summary())
      }
      
      if (input$clDen) {
        sheet5 <- addWorksheet(wb, sheetName = "Denominator")
        writeData(wb, sheet5, df_Den_amplitude())
        
        sheet6 <- addWorksheet(wb, sheetName = "Den_summary")
        writeData(wb, sheet6, df_Den_summary())
      }
      
      
      if (input$clNum & input$clDen) {
        sheet7 <- addWorksheet(wb, sheetName = "custom_ratio")
        writeData(wb, sheet7, df_custom_ratio_amplitude())
        
        sheet8 <-
          addWorksheet(wb, sheetName = "Custom_ratio_summary")
        writeData(wb, sheet8, df_custom_ratio_summary())
      }
  ...
  sheets_in_the_file = eventReactive(eventExpr = {
    input$read_sheets
  },
  valueExpr = {
  ...
  dt_to_shift <- eventReactive(eventExpr = {
    input$read_sheets
    input$sheets
  },
  valueExpr = {
  ...
  observeEvent(input$plots_init_single, {
    output$plot_shift_upper <- renderPlotly({
      req(input$sheets,
          ...
          input$end_t_shift)
      ggplotly_render(
        display_single_plot(dt_to_shift(),
                            input$cellShiftInput,
                            ready = F),
        region = T,
  ...
  observeEvent(input$plots_shift_single, {
    output$plot_shift_lower <- renderPlotly({
      req(input$sheets)
      ggplotly_render(
        display_single_plot(shifted_dataframe(),
                            input$cellShiftInput,
                            ready = F),
        region = T,
        r_min = input$start_t_shift,
  ...
  output$SavePltsShift <- downloadHandler(
    filename = function() {
      filename(input$read_sheets$name, "Shifted.xlsx")
    },
    content = function(file) {
      wb <- createWorkbook()
  ...
  output$SaveAverage <- downloadHandler(
    filename = function() {
      filename(input$read_sheets$name, "Shifted_with_Average.xlsx")
    },
    content = function(file) {
      df_list <- list('first' = init_average(),
                      'second' = shifted_average())
      names(df_list) <-
        c(paste0(input$sheets, '_init'),
          paste0(input$sheets, '_shifted'))
  ...
  cell_to_rotate_value <- reactiveVal(NULL)
  ...
  cell_to_rotate <- eventReactive(eventExpr = {
    ...
    valueExpr = {
      ...
      list_of_numbers <- colnames(dataframe_to_process())

      list_of_numbers <- as.numeric(na.omit(stringr::str_extract(list_of_numbers, "\\b\\D*?0*([1-9][0-9]*)",
                                                                 group = 1)))

      if (!(input$cell_to_plot_to_rotate %in% list_of_numbers)) {
        
        if (cell_to_rotate_value() > input$cell_to_plot_to_rotate) {
          new_value <- max(list_of_numbers[list_of_numbers<=input$cell_to_plot_to_rotate])
        } else if (cell_to_rotate_value() < input$cell_to_plot_to_rotate) {
          
          if (input$cell_to_plot_to_rotate >= max(list_of_numbers)) {
            new_value <- max(list_of_numbers)
          } else {
            new_value <- min(list_of_numbers[list_of_numbers>=input$cell_to_plot_to_rotate])
          }
        
        } else {
          new_value <- list_of_numbers[[1]]
          }
        ...
        new_value <- input$cell_to_plot_to_rotate
      }
      
      cell_to_rotate_value(new_value)

      return(cell_to_rotate_value())
    })
  
  cell_rotated_value <- reactiveVal(NULL)
  ...
  rotated_cell <- eventReactive(eventExpr = {
    ...
    valueExpr = {
      ...
      if (!(input$rotated_plots %in% list_of_numbers)) {
        
        if (cell_rotated_value() > input$rotated_plots) {
          new_value <- max(list_of_numbers[list_of_numbers<=input$rotated_plots])
        } else if (cell_rotated_value() < input$rotated_plots) {
          
          if (input$rotated_plots >= max(list_of_numbers)) {
            new_value <- max(list_of_numbers)
          } else {
            new_value <- min(list_of_numbers[list_of_numbers>=input$rotated_plots])
          }
        ...
        new_value <- input$rotated_plots
      }
      
      cell_rotated_value(new_value)
      
      return(cell_rotated_value())
    })
  ...
  # Rendering datatable related to selected sheet--------------------------
  data_to_rotate <- eventReactive(eventExpr = {
    input$read_curves
    input$data_sheets
  },
  valueExpr = {
   ...
   if (timeStep != 5) {
    ...
    index1 <- length(dfts$Time)/2
    index2 <- index1 + length(dfts$Time)/4
    ...
    updateNumericInput(session = session, 
                       ...
                       max = max(dfts$Time), 
                       step = timeStep,
                       value = dfts$Time[[index2]])
    }
    
    return(read_excel(input$read_curves$datapath, sheet = input$data_sheets))
  })
    ...
    if (input$rotate_part == T) {
      ...
      plot <-
        rotating_plot(
          ...
          st,
          en,
          part = input$rotate_part,
          shift_down = input$rotate_down
        )
  ...
  df2dim_single_and_rotated <-
    ...
    valueExpr = {
      df2dim <-
        getting_a_slice_of_df(dataframe_to_process(), input$cell_to_plot_to_rotate)
  ...
  observeEvent(input$rotate_single_plot, {
    ...
    dataframe_to_process_new <-
      replace_columns_in_dfs(dataframe_to_process(), df2dim_single_and_rotated())
  ...
  df2dim_single_and_rotated_part <-
    ...
    valueExpr = {
      df2dim <- df2dim_single_and_rotated()
      
      st <- input$line_start
      en <- input$line_end
      
      df2dim <-
        rotating_plot(df2dim,
                      st,
                      en,
                      part = TRUE,
  ...
  rotated_plot_single <- eventReactive(eventExpr = {
    input$rotate_single_plot_part
    input$rotate_single_down
    input$rotate_single_plot
    ...
    input$flat_end
                                       ...
                                       valueExpr = {
                                         pl <- df2dim_single_and_rotated_part()
  ...
  # Buttons to Exclude/Undo/Include/Reset cells
  observeEvent(ignoreInit = TRUE,
               list(input$rotate_single_plot, input$rotate_single_plot_part),
               {
                 req(data_to_rotate())
                 ...
                 rmcellValues$cells_altered_manually <-
                   unique(c(
                     shiny::isolate(rmcellValues$cells_altered_manually),
                     shiny::isolate({
                       finding_cell_name(data_to_rotate(),
                                         input$cell_to_plot_to_rotate)
                     })
                   ))
  ...
  observeEvent(input$reset_current_changes, {
    ...
    rmcellValues$cells_altered_manually <-
      rmcellValues$cells_altered_manually[-grep(
        finding_cell_name(data_to_rotate(),
                          input$cell_to_plot_to_rotate),
        rmcellValues$cells_altered_manually
      )]
    ...
    output$list_of_cells_altered_manually <- renderPrint({
      if (length(rmcellValues$cells_altered_manually) == 0) {print("Reset! No manually changes have been made!")
        } else {
      gtools::mixedsort(rmcellValues$cells_altered_manually, decreasing = F)}
    })
  })
  ...
  observeEvent(input$reset_all_changes, {
    ...
    output$list_of_cells_altered_manually <- renderPrint({
      print("Reset! No manually changes have been made!")
    })
  ...
  observeEvent(input$rotate_all_other_cells, {
    ...
    output$list_of_cells_altered_manually <- renderPrint({
      ...
      if ((baselinev != input$rotate_baseline_as_well) |
          (shiftv != input$rotate_single_down)) {
        text <- ("Press 'Reset all' button to recalculate!")
        print(text)
        
      } else if (length(rmcellValues$cells_altered_manually) == 0) {
        print("Rotated automatically. No traces were changed manually.")  
      } else {
        gtools::mixedsort(rmcellValues$cells_altered_manually, decreasing = F)
      }
  ...
  observeEvent(input$rotate_all_cells_from_scratch, {
    ...
    dataframe_to_process_new2 <- rotate_all(
      data_to_rotate(),
      rmcellValues$cells_altered_manually,
      lower_base = input$line_start,
      upper_base = input$line_end,
    ...
    output$list_of_cells_altered_manually <- renderPrint({
      ...
      if (shiftv) {
        text <-
          paste0(text, ' and baseline shifted to match the next value')
      } else {
  ...
  observeEvent(input$reset_plot, {
    updateSwitchInput(inputId = "mark_line_to_rotate",
                      value = TRUE)
    
    updateSwitchInput(inputId = "rotate_average",
                      value = FALSE)
    
    updateSwitchInput(inputId = "rotate_part",
                      value = FALSE)
    
    updateSwitchInput(inputId = "rotate_down",
                      value = TRUE)
  ...
  output$SaveFinal <- downloadHandler(
    filename = function() {
      filename(input$read_curves$name, "Rotated.xlsx")
    },
    content = function(file) {
      pattern <- grep('([Tt]ime\\s?)', colnames(dataframe_to_process()))

      dataframe_to_process <-
        cbind(dataframe_to_process()[pattern],
              dcm((dataframe_to_process()[-pattern])))

      df_list <- list(
        'first' = dataframe_to_process,
      ...
      names(df_list) <- c(
        paste0(input$sheets, 'data_rotated'),
  ...
  # Render plot
  plot_average_output2 <-
    eventReactive(eventExpr = {
      input$plot_rotated_result
      input$rotate_average
      input$rotate_part
      input$rotate_down
    ...
    valueExpr = {
      ...
      curve <-
        ggplotly_render(
          average_curve(dataframe_to_process()),
          baseline = T,
  ...
  all_curves_area <-
    ...
    valueExpr = {
      ...
      result_area_df <-
        summarize(group_by(area_df, cell),
  ...
  all_polygons_df <-
    ...
    valueExpr = {
      ...
      for (idx in 2:ncol(rotated_df)) {
        ...
        sd = SD(subset(
          df_slice,
          (Time >= input$baseline_start) &
            (Time <= input$baseline_end)
        )[[2]])
        cv = (sd / b) * 100
        maximum = max(subset(
          df_slice,
          (Time >= input$area_start) & (Time <= input$area_end)
        )[[2]])
  ...
  result_statistics_df <-
    eventReactive(eventExpr = {
      input$calculate_area_statistics
    },
    ...
    valueExpr = {
      if (!is.null(result_table())) {
        ...
        res_statistics <- data.frame(
          ...
          CV_Area_pct = decim(100 * SD(res_table$Area) /
                                mean(res_table$Area), 1),
          Mean_Difference = decim(mean(res_table$Difference)),
          SD_Difference = decim(SD(res_table$Difference)),
          CV_Difference_pct = decim(
            100 * SD(res_table$Difference) / mean(res_table$Difference),
            1
          ),
          ...
          CV_A_by_d_pct = decim(
            100 * SD(res_table$Area_by_Dif) / mean(res_table$Area_by_Dif),
            1
          ),
          Mean_Baseline_av = decim(mean(res_table$Baseline_average)),
          CV_Baseline_av_pct = decim(
            100 * SD(res_table$Baseline_average) / mean(res_table$Baseline_average),
            1
          )
        )
  ...
  output$SaveAreaStatistics <- downloadHandler(
    filename = function() {
      filename(input$read_curves$name, "Area_Statistics.xlsx")
    },
    content = function(file) {
      df_list <- list(
        'area' = result_table(),
